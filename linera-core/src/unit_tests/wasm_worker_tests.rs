    ...
    ViewError: From<S::StoreError>,
{
    ...
    let contract_bytecode = BlobContent::load_from_file(contract_path).await?;
    let service_bytecode = BlobContent::load_from_file(service_path).await?;

    let contract_blob_hash = contract_bytecode.clone().with_bytecode_blob_id().id().hash;
    let service_blob_hash = service_bytecode.clone().with_bytecode_blob_id().id().hash;
    ...
    let create_block = make_child_block(&publish_certificate.value)
        .with_timestamp(4)
