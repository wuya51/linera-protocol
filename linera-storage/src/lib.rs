...
pub trait Storage: Sized {
    ...
    async fn load_contract(
        ...
        let contract_bytecode_blob_id =
            BlobId::new_bytecode_from_hash(application_description.bytecode_id.contract_blob_hash);
        let contract_blob = self.read_blob(contract_bytecode_blob_id).await?;
        Ok(Arc::new(
            WasmContractModule::new(contract_blob.into_inner(), wasm_runtime).await?,
        ))
    }
    ...
    async fn load_service(
        ...
        let service_bytecode_blob_id =
            BlobId::new_bytecode_from_hash(application_description.bytecode_id.service_blob_hash);
        let service_blob = self.read_blob(service_bytecode_blob_id).await?;
        Ok(Arc::new(
            WasmServiceModule::new(service_blob.into_inner(), wasm_runtime).await?,
        ))
    }
