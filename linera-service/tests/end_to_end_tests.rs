...
impl NonFungibleApp {
    async fn get_nft(&self, token_id: &String) -> Result<non_fungible::Nft, serde_json::Error> {
        let query = format!(
            "nfts {{ entry(key: {}) {{ value {{ tokenId, owner, name, minter, payload }} }} }}",
            token_id.to_value()
        );
        let response_body = self.0.query(&query).await.unwrap();
        serde_json::from_value(response_body["nfts"]["entry"]["value"].clone())
    }

    async fn get_owned_nfts(
        &self,
        owner: &AccountOwner,
    ) -> Result<Vec<non_fungible::TokenId>, serde_json::Error> {
        let query = format!("owners {{ entry(key: {}) {{ value }} }}", owner.to_value());
        let response_body = self.0.query(&query).await.unwrap();
        serde_json::from_value(response_body["owners"]["entry"]["value"].clone())
    }

    async fn mint(&self, name: &String, extra_data: &Vec<u8>) -> Value {
        let mutation = format!(
            "mint(name: {}, extraData: {})",
            name.to_value(),
            extra_data.to_value(),
...
async fn test_wasm_end_to_end_non_fungible(config: impl LineraNetConfig) {
    use non_fungible::{Nft, NonFungibleTokenAbi};
    ...
    let nft1_id = Nft::create_token_id(
        &chain1,
    ...
    let nft2_id = Nft::create_token_id(
        &chain2,
        &application_id.forget_abi(),
        &nft2_name,
        &nft2_minter,
